...
final class CircuitDossier
{
    ...
    private static function exigerControleMedicalPertinent(EtatDossier $etat): void
    {
        if (!$etat->nature->exigeControleMedical()) {
            throw TransitionRefusee::parce(sprintf('Un sinistre « %s » ne relève pas du contrôle médical.', $etat->nature->value));
        }
    }
    ...
    private static function exigerDossierComplet(EtatDossier $etat): void
    {
        if ([] !== $etat->piecesManquantes) {
            throw TransitionRefusee::parce(sprintf('Il manque %d pièce(s) obligatoire(s) : %s.', count($etat->piecesManquantes), implode(', ', array_slice($etat->piecesManquantes, 0, 5))));
        }

        if (!$etat->repartitionEstComplete()) {
            throw TransitionRefusee::parce(sprintf('La répartition entre bénéficiaires totalise %s %% au lieu de 100 %%.', number_format($etat->quotePartTotaleCentiemes / 100, 2, ',', ' ')));
        }
    }
    ...
    private static function exigerLiquidation(EtatDossier $etat): void
    {
        if ($etat->capitalLiquideXaf <= 0) {
            throw TransitionRefusee::parce('Le capital doit être liquidé avant de valider la prise en charge.');
        }

        if (!$etat->repartitionEstComplete()) {
            throw TransitionRefusee::parce('La répartition entre bénéficiaires doit totaliser 100 %.');
        }
    }
    ...
    private static function exigerMontantsVentiles(EtatDossier $etat): void
    {
        if ($etat->beneficiairesSansMontant > 0) {
            throw TransitionRefusee::parce(sprintf('%d bénéficiaire(s) sans montant arrêté : relancez la liquidation.', $etat->beneficiairesSansMontant));
        }
    }
    ...
    private static function exigerTousLesReglementsPayes(EtatDossier $etat): void
    {
        if ($etat->beneficiairesNonRegles > 0) {
            throw TransitionRefusee::parce(sprintf("%d bénéficiaire(s) n'ont pas encore été réglés.", $etat->beneficiairesNonRegles));
        }
    }
    ...
    private static function exigerMotif(?MotifRejet $motif): void
    {
        if (!$motif instanceof MotifRejet) {
            throw TransitionRefusee::parce('Un rejet ou un classement sans suite doit être motivé.');
        }
    }
}
